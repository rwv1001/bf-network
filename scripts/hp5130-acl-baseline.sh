...
emit(f" rule 65 permit udp source {network_word}.{management_vlan}.0 0.0.0.255 destination {portal_ip} 0 destination-port eq syslog")
emit(f" rule 66 permit tcp source {network_word}.{management_vlan}.0 0.0.0.255 destination {portal_ip} 0 destination-port eq cmd")
