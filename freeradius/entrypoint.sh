...
if [ -f /etc/freeradius/clients.conf.template ]; then
    ...
    sed "s/\${RADIUS_SECRET}/${RADIUS_SECRET}/g" /etc/freeradius/clients.conf.template > /etc/freeradius/clients.conf
...
if [ -f /etc/freeradius/proxy.conf.template ]; then
    sed "s/\${RADIUS_SECRET}/${RADIUS_SECRET}/g" /etc/freeradius/proxy.conf.template > /etc/freeradius/proxy.conf
    echo "proxy.conf generated with RADIUS_SECRET from environment"
fi
