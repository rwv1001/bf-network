// Working version: Get MAC from lease, not query
...
int load(LibraryHandle& handle) {
    std::cout << "DNS Hijack Hook WORKING: Loaded" << std::endl;
    return 0;
}

int unload() {
    std::cout << "DNS Hijack Hook WORKING: Unloaded" << std::endl;
    return 0;
}
...
int lease4_select(CalloutHandle& handle) {
    try {
        ...
        // Get MAC from lease, not from query4
        ...
        if (host) {
            std::cout << "DNS Hijack Hook: Device " << mac << " is REGISTERED - unhijack" << std::endl;
            std::cout.flush();
            call_script("unhijack", ip);
        } else {
            std::cout << "DNS Hijack Hook: Device " << mac << " is UNREGISTERED - hijack" << std::endl;
            std::cout.flush();
